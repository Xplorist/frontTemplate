...
/*------------------其它數據-----------------------*/		
	...
	methods: {
		...
		open_add: function() {
			var _self = this;
			window.location.href = 'ERX_CS_Edit.html?page_type=add&project_id=' + _self.project_id
								+ '&project_name=' + _self.project_name;
		},
		// 產品評估報告list查詢
		query_CS_Main_list: function(pageIndex) {
			var _self = this;
			axios({
				  method: 'post',
				  url: '/ePDMWeb/ERX_CS/query_CS_Main_list.x',
				  data: {
					  	'project_name': _self.project_name,
					...
					_self.cs_main_list = data.cs_main_list;
					...
					myPagi = new myPagination({
					    ...
					    getPage: function(pageIndex) {
					    	// 点击分页按钮请求数据
					    	_self.query_CS_Main_list(pageIndex);
					    }
					});
				});
		},
		...
		open_edit: function(pkid) {
			var _self = this;
			window.location.href = 'ERX_CS_Edit.html?page_type=edit&pkid='+ pkid +'&project_id=' + _self.project_id
								+ '&project_name=' + _self.project_name;
		},
		// 打開展示
		open_show: function(pkid) {
			var _self = this;
			window.location.href = 'ERX_CS_Show.html?page_type=show&pkid='+ pkid +'&project_id=' + _self.project_id
								+ '&project_name=' + _self.project_name;
		},
		// 查詢審核記錄
